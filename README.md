...
relational DB are the type of DB that count with 2 more tables where u can store the info in columns and rows, and SQL is one of the many languages in which u cna write DB

2. Why do tables need a `primary key`?

that way we can refer to an specific entry on that table without touch any other
...
we need another table that has the record between both tables ex, a tbale that store the table1.id with table2,id
